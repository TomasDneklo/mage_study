    ...
    extends Mage_Core_Helper_Abstract
{
     ...
     * @param string $scope the reques key for file
     * @return bool|string
     */
    public function uploadImage($scope)
    {
        ...
        if($adapter->isUploaded($scope)){
            ...
            if(!$adapter->isValid($scope)){
                Mage::throwException(Mage::helper('study_news')->__('Uploaded image is not valid'));
            }
            $uplopad = new Varien_File_Uploader($scope);
            $uplopad->setAllowCreateFolders(true);
            $uplopad->setAllowedExtensions($this->_allowedExtensions);
            $uplopad->setAllowRenameFiles(true);
            $uplopad->setFilesDispersion(false);
            if($uplopad->save($this->getBaseDir())){
                return $uplopad->getUploadedFileName();
            }
        }
        return false;
    }
     ...
     * @return bool|string
     */
    public function resize(Study_News_Model_News $item, $width, $height = null)
    {
        if(!$item->getImage()){
            return false;
        }
